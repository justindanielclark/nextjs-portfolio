...
function Canvas() {
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      const OrbMap = new SpatialHashGrid<Orb>(window.innerWidth, window.innerHeight, Orb.qualifyingNearDistance);
      if (ctx) {
        const onWindowResize = (
          ...
          setDebounce(() => {
            ...
            orbMap.reset(window.innerWidth, window.innerHeight, Orb.qualifyingNearDistance);
            Orb.canvasHeight = window.innerHeight;
            Orb.canvasWidth = window.innerWidth;
            const numOrbs = Math.floor((window.innerWidth * window.innerHeight) / ORB_COEFF);
            for (let i = 0; i < numOrbs; i++) {
              orbMap.add(new Orb(1));
            }
          }, 100);
        };
        ...
        //   }
        // };
        const drawLineBetweenOrbs = (orb1: Orb, orb2: Orb, percentage: number) => {
          if (ctx) {
            ctx.beginPath();
        ...
        const renderFrame = () => {
          ...
          if (ctx) {
            ...
            const connections = OrbMap.reduce((acc, cur) => {
              const neighbors = OrbMap.getPossibleNeighbors(cur, Orb.qualifyingNearDistance);
              neighbors.forEach((neighborOrb) => {
                ...
                if (difference > 0) {
                  if (acc.get(neighborOrb) === null || !acc.get(neighborOrb)?.has(cur)) {
                    if (acc.get(cur)) {
  ...
  return <canvas ref={canvasRef} className="absolute z-0 top-0 left-0" />;
}
