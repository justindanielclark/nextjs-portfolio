...
export default class SpatialHashGrid<T extends hasPosition & hasPositionUpdate> {
  ...
  private initialize(width: number, height: number, divisional: number) {
    ...
    this.cellDiagonalLength = Math.sqrt(Math.pow(this.cellWidth / 2, 2) + Math.pow(this.cellHeight / 2, 2));
    for (let i = 0; i < x; i++) {
      this.cells.push([]);
      for (let j = 0; j < y; j++) {
        this.cells[i].push({
  ...
  public add(obj: T): void {
    ...
    if (this.cells && this.cells[x] && this.cells[x][y] && this.cells[x][y].items) {
  ...
  public reduce<K>(callback: (accumulator: K, currentValue: T) => K, returnObj: K): K {
  ...
  public getPossibleNeighbors(obj: T, distance: number): Array<T> {
    ...
    for (let x = cellPos.x - checkOffset; x < cellPos.x + checkOffset; x++) {
      if (x >= 0 && x < this.cells.length) {
        for (let y = cellPos.y - checkOffset; y < cellPos.y + checkOffset; y++) {
  ...
  public isNearby(obj1: T, obj2: T, distanceSquared: number): boolean {
    return distanceSquared < Math.pow(obj2.getX() - obj1.getX(), 2) + Math.pow(obj2.getY() - obj1.getY(), 2);
  }
  public distBetweenSquared(obj1: T, obj2: T): number {
    return Math.pow(obj2.getX() - obj1.getX(), 2) + Math.pow(obj2.getY() - obj1.getY(), 2);
  }
