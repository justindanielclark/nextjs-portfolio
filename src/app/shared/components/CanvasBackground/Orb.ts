export default class Orb implements hasPosition, hasPositionUpdate {
  ...
  static baseRadius = 1;
  static qualifyingNearDistance = 300;
  ...
  constructor(radius: number) {
    ...
    this.radius = radius;
    const { x, y, xVel, yVel } = Orb.generateInitialStartAndVelocity(this.vector);
    this.x = x;
    this.y = y;
    this.xVel = xVel;
    this.yVel = yVel;
  ...
  public generateNewStartingPosition(): void {
    const { x, y, xVel, yVel } = Orb.generateNewStartAndVelocity(Orb.limit, this.vector);
    this.x = x;
    this.y = y;
    this.xVel = xVel;
    this.yVel = yVel;
  ...
  public static willOrbsBeNearEachOther(orb1: Orb, orb2: Orb): boolean {
    ...
    const c = dx * dx + dy * dy - Orb.qualifyingNearDistance * Orb.qualifyingNearDistance;
  ...
  private static generateInitialStartAndVelocity(vector: number): Position & Speed {
  ...
  private static generateNewStartAndVelocity(limit: number, vector: number): Position & Speed {
    ...
    } else if (startingPoint < 6 * limit + 2 * Orb.canvasWidth + Orb.canvasHeight) {
